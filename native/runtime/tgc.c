...
static size_t tgc_probe(tgc_t *gc, size_t i, size_t h) {
    long v = i - (h - 1);
    if (v < 0) { v = gc->n_slots + v; }
    return v;
}

static tgc_ptr_t *tgc_get_ptr(tgc_t *gc, void *ptr) {
    size_t i, j, h;
    i = tgc_hash(ptr) % gc->n_slots;
    j = 0;
    while (1) {
        h = gc->items[i].hash;
...
static const size_t tgc_primes[TGC_PRIMES_COUNT] = {
        0, 1, 5, 11, 23, 53, 101, 197, 389, 683, 1259, 2417, 4733, 9371, 18617,
        37097, 74093, 148073, 296099, 592019, 1100009, 2200013, 4400021, 8800019
};
...
static int tgc_rehash(tgc_t *gc, size_t new_size) {
    ...
    gc->n_slots = new_size;
    gc->items = calloc(gc->n_slots, sizeof(tgc_ptr_t));
...
static int tgc_resize_more(tgc_t *gc) {
    size_t new_size = tgc_ideal_size(gc, gc->n_items);
    size_t old_size = gc->n_slots;
...
static void tgc_mark_ptr(tgc_t *gc, void *ptr) {
    ...
    if ((uintptr_t) ptr < gc->min_ptr
        || (uintptr_t) ptr > gc->max_ptr) { return; }
...
static void tgc_mark_stack(tgc_t *gc) {
    ...
    if (bottom < top) {
        for (p = top; p >= bottom; p = ((char *) p) - sizeof(void *)) {
...
void tgc_sweep(tgc_t *gc) {
    ...
    gc->frees = realloc(gc->frees, sizeof(tgc_ptr_t) * gc->n_frees);
    ...
    gc->m_items = gc->n_items + (size_t) (gc->n_items * gc->sweep_factor) + 1;
...
void tgc_start(tgc_t *gc, void *stack) {
    gc->bottom = stack;
    gc->paused = 0;
    gc->n_items = 0;
    gc->n_slots = 0;
...
static void *tgc_add(tgc_t *gc, void *ptr, size_t size, int flags, void(*dtor)(void *)) {
    ...
    if (tgc_resize_more(gc)) {
        ...
        if (!gc->paused && gc->n_items > gc->m_items) {
            tgc_run(gc);
        }
        return ptr;
    } else {
...
static void tgc_rem(tgc_t *gc, void *ptr) {
    tgc_rem_ptr(gc, ptr);
    tgc_resize_less(gc);
    gc->m_items = gc->n_items + gc->n_items / 2 + 1;
}
